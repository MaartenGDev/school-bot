...
class SlackClient implements ParserInterface
{
    ...
    /**
     * Reset the colors when all going to a next day
     *
     * @return void
     */
     ...
     *
     * @param string $description The name of the day.
     *
     * @return string
     */
    public function parseDayAndWeek($description)
    {
        ...
        $day = $this->translations->search(
            function ($translations) use ($day) {
                return ($translations->search($day) === true);
            }
        ) ?: 'Monday';
    ...
    public function parse($week)
    {
        ...
        json_decode($week))->groupBy(
            ...
            function ($lessons) {
                $this->resetColors();
                return $lessons->groupBy('long_name')->map(
                    function ($lessons) {
                        return $lessons->map(
                            function ($lesson) {
